...
export default {
    template: `
        <div style="min-height: 90vh; width: 100%; background-color: var(--bg-color, #121212); color: var(--text-color, #ffffff); padding: 2rem 1.5rem; box-sizing: border-box;">
            <div style="max-width: 900px; margin: 0 auto;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 1.25rem;">
                    <div>
                        <h1 style="font-size: 2rem; font-weight: 800; margin: 0; color: #ffffff;">Editor Review Queue</h1>
                        <p style="opacity: 0.7; font-size: 0.95rem; margin-top: 0.35rem; color: #e0e0e0;">Approve or reject pending record submissions.</p>
                    </div>
                ...
                <!-- Access Denied Guard -->
                <div v-if="!store.profile?.is_editor" style="text-align: center; padding: 4rem 1.5rem; background: rgba(206,17,38,0.15); border: 1px solid rgba(206,17,38,0.3); border-radius: 14px;">
                    <h3 style="color: #ce1126; font-size: 1.35rem; font-weight: 800; margin-bottom: 0.5rem;">Access Restricted</h3>
                    <p style="opacity: 0.85; font-size: 0.95rem; color: #ffffff;">You need Editor permissions to view and process submissions.</p>
                </div>

                <!-- Loading State -->
                <div v-else-if="loading" style="text-align: center; padding: 4rem 1rem; opacity: 0.7; font-size: 1.1rem; font-weight: 700; color: #ffffff;">
                    Loading pending queue...
                </div>

                <!-- Empty State -->
                <div v-else-if="submissions.length === 0" style="text-align: center; padding: 4rem 1.5rem; background: #1a1a1a; border: 1px solid rgba(255,255,255,0.08); border-radius: 14px;">
                    <p style="font-size: 1.25rem; font-weight: 800; color: #00FF80; margin-bottom: 0.35rem;">Queue Clear!</p>
                    <p style="opacity: 0.7; font-size: 0.9rem; color: #cccccc;">No pending record submissions waiting for review.</p>
                </div>
                ...
                <div v-else style="display: flex; flex-direction: column; gap: 1.25rem;">
                    <div 
                        ...
                        :key="sub.id" 
                        style="background: #1e1e1e; border: 1px solid rgba(255,255,255,0.12); border-radius: 14px; padding: 1.5rem; display: flex; flex-direction: column; gap: 1rem; box-shadow: 0 8px 24px rgba(0,0,0,0.5);"
                    >
                        <!-- Card Top Row -->
                        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255,255,255,0.08); padding-bottom: 0.85rem;">
                            <div>
                                ...
                                <span style="font-size: 1.1rem; font-weight: 700; opacity: 0.9; margin-left: 0.6rem; color: #ffffff;">({{ sub.percent }}%)</span>
                            </div>
                            <span style="font-size: 0.75rem; opacity: 0.5; font-family: monospace; color: #aaaaaa;">ID: {{ sub.id.slice(0, 8) }}</span>
                        </div>

                        <!-- Player Info Header -->
                        <div style="display: flex; align-items: center; gap: 1rem; background: rgba(255,255,255,0.05); padding: 0.85rem 1.1rem; border-radius: 10px;">
                            <img 
                            ...
                            />
                            <div>
                                <span style="font-weight: 800; font-size: 1.05rem; display: block; color: #ffffff;">
                                    {{ sub.profiles?.username || 'Player' }}
                                </span>
                                <span style="font-size: 0.8rem; opacity: 0.6; color: #cccccc;">
                        ...
                        <!-- Submission Details -->
                        <div style="font-size: 0.9rem; opacity: 0.85; line-height: 1.4; color: #dddddd;">
                            <strong style="color: #ffffff;">Details:</strong> {{ sub.notes || 'None provided' }}
                        </div>
